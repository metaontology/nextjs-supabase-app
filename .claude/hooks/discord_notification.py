...
권한 요청 또는 알림 발생 시 Discord로 메시지를 전송합니다.
"""
...
def main() -> None:
    ...
    footer_text = f"📁 {project_name}  |  🕐 {now}"
    if notification_type:
        footer_text += f"  |  🏷️ {notification_type}"

    payload = {
        "username": "Claude Code Bot",
        "embeds": [
            {
                "title": f"🔔 {title}",  # title 필드 동적 반영
                "color": 0xFEE75C,  # 노란색 - 주의/확인 필요
                "fields": [
                    {
                        "name": "메시지",
                        "value": message,  # message 레이블 추가
                        "inline": False,
                    },
                ],
                "footer": {
                    "text": footer_text,  # notification_type 포함
                },
            }
        ],
    }
